...
process.env.TEST_QUEUE_NAME = 'code-review-test-' + Date.now();
...
// Dynamically import the service, queue, and worker so they pick up the environment variables
const { queueReview } = await import('../src/service/codeReviewService.js');
const { codeReviewQueue } = await import('../src/queues/codeReviewQueue.js');
await import('../src/workers/codeReviewWorker.js');

const TEST_DB_URI = process.env.DB_URI || 'mongodb://127.0.0.1:27017/ims_test';

// Named constants for timeouts and wait times
...
async function runTests() {
  ...
  const originalCodeReviewApiUrl = process.env.CODE_REVIEW_API_URL;
  const originalFetch = global.fetch;

  try {
    // Set default api url for test runs
    process.env.CODE_REVIEW_API_URL = 'https://api.example.com/code-review';

    // Mock global fetch for testing external API calls
    global.fetch = async (url, options) => {
      if (url && url.includes('code-review')) {
        if (url.includes('65499')) {
          throw new Error('fetch failed');
        }
        const marksObtained = Math.round((5 + Math.random() * 5) * 10) / 10;
        const codeQualityScore = Math.round(5 + Math.random() * 5);
        ...
        return {
          ok: true,
          text: async () => JSON.stringify({ marksObtained, codeQualityScore, feedback }),
          json: async () => ({ marksObtained, codeQualityScore, feedback })
        };
      }
    ...
    try {
      ...
      // Drop the stale index that is no longer defined in student.model.js
      try {
        await mongoose.connection.collection('students').dropIndex('rollnumber_1');
        console.log('✓ Stale database index "rollnumber_1" dropped successfully.');
      } catch (err) {
        // Index might not exist or already be dropped, ignore error
      }
    } catch (error) {
    ...
    await Promise.all([
      User.deleteMany({ email: { $in: [/@test-codereview\.com$/, 'mohdsaadkhan073+student@gmail.com'] } }),
    ...
    const activeStudentUser = await User.create({
      email: 'mohdsaadkhan073+student@gmail.com',
      mobileNo: '8888888888',
      name: 'Test Student',
      password: 'password123',
    ...
    const testAssignment = await Assignment.create({
      ...
      submissionDeadline: new Date(Date.now() + ONE_HOUR_MS), // 1 hour from now (On Time)
    ...
    const lateSubmission = await AssignmentSubmission.create({
      ...
      submittedAt: new Date(Date.now() + TWO_HOURS_MS), // 2 hours from now (Late)
    ...
    // Case D: Duplicate queueing prevention
    const duplicateTestSubmission = await AssignmentSubmission.create({
      studentId: testStudent._id,
      assignmentId: testAssignment._id,
      gitSubmissionLink: 'https://github.com/test-owner/test-repo-dup',
      repoName: 'test-repo-dup',
      branchName: 'main',
    ...
    try {
      await queueReview(duplicateTestSubmission._id);
      await queueReview(duplicateTestSubmission._id);
      console.error('✗ Failure: Did not reject duplicate review queue request');
    } catch (err) {
      if (err.message === 'Review already queued.') {
        console.log('✓ Passed: Correctly prevented duplicate review queuing:', err.message);
      } else {
        console.error('✗ Failure: Failed with unexpected error:', err.message);
      }
    }
    ...
    if (redisAvailable) {
      ...
      console.log(`Waiting ${WORKER_WAIT_MS / 1000} seconds for worker to process job...`);
      ...
      console.log(`Waiting ${WORKER_WAIT_MS / 1000} seconds for worker...`);
      await new Promise((resolve) => setTimeout(resolve, WORKER_WAIT_MS));
      ...
      // To simulate API failures, we set CODE_REVIEW_API_URL to a non-existent port
      ...
      console.log(`Waiting ${RETRY_WAIT_MS / 1000} seconds for worker retries and exhaustion...`);
    ...
    if (originalCodeReviewApiUrl !== undefined) {
      process.env.CODE_REVIEW_API_URL = originalCodeReviewApiUrl;
    } else {
      delete process.env.CODE_REVIEW_API_URL;
    }
