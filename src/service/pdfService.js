...
export async function generatePortfolioPDF(studentId, _batchId, res) {
  // Find the student document first using either studentId (Student ID) or studentId (User ID)
  ...
  const resolvedUserId = student.userId;

  const [user, ledgerEntries, submissions] = await Promise.all([
    User.findById(resolvedUserId).select('name email mobileNo profileStatus').lean(),
    StudentLedger.find({ studentId: resolvedStudentId, deletedAt: null }).sort({ createdAt: -1 }).limit(10).lean(),
    AssignmentSubmission.find({ studentId: resolvedStudentId })
  ...
  const metrics = await getAllMetrics(resolvedStudentId, student.batchId);
